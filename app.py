...
kit.servo[2].set_pulse_width_range(900, 2000)
...
def move_servo_channel2():
    ...
    # Accessing JSON data using the get method
    direction = request.json.get('direction')
    step = request.json.get('step')
    
    # Check if 'direction' and 'step' are present in the request data
    if direction is None or step is None:
        return jsonify({'error': 'Invalid request. Missing direction or step.'}), 400
...
#        pass  # Replace 'pass' with your actual motor control code
        ...
        gpio.output(24, True)
    # Return a response
    return jsonify({'status': 'Backing Up...'})
